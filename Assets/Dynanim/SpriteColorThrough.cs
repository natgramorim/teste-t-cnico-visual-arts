...
/// <summary>
/// Colore a image pela lista de cores
/// </summary>
public class SpriteColorThrough : MonoBehaviour
{
    private enum Modo
    {
        Float,
    ...
    private IEnumerator coloring()
    {
            ...
            if (t < 1)
            {
                switch (modo)
                {
                    case Modo.Float:
                        ...
                        //stepT = StepT(Step(step), Step(step + 1), portion, RealT(t));
                        //Debug.Log("cor 1: " + Step(step) + ", cor 2:" + Step(step + 1) + ", t:" + stepT);
                        spriteRenderer.color = Color.Lerp(cor1, cor2, stepT);
                        break;

                    case Modo.Int:
                        step = Mathf.FloorToInt(t / portion);
                        spriteRenderer.color = cores[Step(step)];
                        break;
                }
            }
        ...
        spriteRenderer.color = cores[destino];
    ...
    {
        ColorSet(primeira, última, duração, onColorOutEnd);
    }
